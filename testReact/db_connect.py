...
class DatabaseConnection:
    def __init__(self):
        ...
        try:
            self.connection = psycopg2.connect("dbname='MLB' user='medeni' host='localhost' password='tester23' port='5432'")
            self.connection.autocommit = True
            self.cursor = self.connection.cursor()
            pprint("Coneccted")
            
           # "dbname='test_db' user='medeni' host='localhost' password='tester23' port='5432'
        except:
